    ...
    extends TestCase
{
    ...
    /**
     * RFC 9579 sec. 5: "The length of the key generated by the used KDF ... SHOULD be the same size as
     * the HMAC function output size", the worked example being a 32-octet key for an HMAC-SHA-256
     * scheme. The keystore's message-authentication scheme is HMAC-SHA-512, so its KDF keyLength is
     * 64; up to release 1.86 it asked for 256, four times the output size.
     */
    public void testDefaultPbmac1KeyLength()
        throws Exception
    {
        KeyStore sourceP12 = KeyStore.getInstance("PKCS12-PBMAC1", "BC");
        sourceP12.load(new ByteArrayInputStream(PKCS12TestData._non_PBMAC1_PKCS12), passwd);
        ...
        Pfx pfx = Pfx.getInstance(bOut.toByteArray());
        PBMAC1Params pbmac1Params = PBMAC1Params.getInstance(
            pfx.getMacData().getMac().getAlgorithmId().getParameters());
        PBKDF2Params pbkdf2Params = PBKDF2Params.getInstance(
            pbmac1Params.getKeyDerivationFunc().getParameters());

        assertEquals(PKCSObjectIdentifiers.id_hmacWithSHA512, pbmac1Params.getMessageAuthScheme().getAlgorithm());
        assertEquals(BigInteger.valueOf(64), pbkdf2Params.getKeyLength());

        // the same default reached through the public builder that configures this
        PKCS12StoreParameter.PBMAC1WithPBKDF2Builder pbmacBuilder = PKCS12StoreParameter.pbmac1WithPBKDF2Builder();
        pbmacBuilder.setSalt(new byte[20]);

        assertEquals(BigInteger.valueOf(64), PBKDF2Params.getInstance(
            PBMAC1Params.getInstance(pbmacBuilder.build().getParameters())
                .getKeyDerivationFunc().getParameters()).getKeyLength());

        // and the file still loads
        KeyStore reloaded = KeyStore.getInstance("PKCS12-PBMAC1", "BC");
        reloaded.load(new ByteArrayInputStream(bOut.toByteArray()), "4567".toCharArray());
        assertNotNull(reloaded.getCertificate("alpha"));
    }

    /**
     * Compatibility half, and the reason the keyLength upper bound stays well above the HMAC output
     * size: a PFX asking for the 256-octet MAC key releases up to 1.86 wrote has to keep verifying.
     */
    public void testLegacy256OctetMacKeyStillLoads()
        throws Exception
    {
        KeyStore sourceP12 = KeyStore.getInstance("PKCS12-PBMAC1", "BC");
        sourceP12.load(new ByteArrayInputStream(PKCS12TestData._non_PBMAC1_PKCS12), passwd);
        ...
        PKCS12StoreParameter.PBMAC1WithPBKDF2Builder pbmacBuilder = PKCS12StoreParameter.pbmac1WithPBKDF2Builder();
        pbmacBuilder.setSalt(new byte[20]);
        pbmacBuilder.setKeySize(256);

        ByteArrayOutputStream bOut = new ByteArrayOutputStream();
        PKCS12StoreParameter.Builder builder = PKCS12StoreParameter.builder(bOut, "4567".toCharArray());
        builder.setMacAlgorithm(pbmacBuilder.build());
        pkcs12.store(builder.build());

        Pfx pfx = Pfx.getInstance(bOut.toByteArray());
        assertEquals(BigInteger.valueOf(256), PBKDF2Params.getInstance(
            PBMAC1Params.getInstance(pfx.getMacData().getMac().getAlgorithmId().getParameters())
                .getKeyDerivationFunc().getParameters()).getKeyLength());

        KeyStore reloaded = KeyStore.getInstance("PKCS12-PBMAC1", "BC");
        reloaded.load(new ByteArrayInputStream(bOut.toByteArray()), "4567".toCharArray());

        assertNotNull("a PFX written by an earlier release no longer loads", reloaded.getCertificate("alpha"));
    }
